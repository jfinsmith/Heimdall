/**
 * Cadet Reports — coordinators file academic-action reports (exam failure,
 * dismissal, etc.) against a chosen academy class, then print a memorandum that
 * reproduces the official PHSC/FDLE letter. Report types are discipline-scoped
 * (the first four are Law Enforcement only).
 */
...
const isLawEnforcement = (a?: WithId<AcademyDoc>) =>
...
export function CadetReportsPage() {
  ...
  const { data: roster } = useCollection<RosterMemberDoc>(academyId ? `academies/${academyId}/roster` : null, [orderBy('no')], [academyId]);
  const { data: reports } = useCollection<AcademyReportDoc>(academyId ? `academies/${academyId}/reports` : null, [orderBy('createdAt', 'desc')], [academyId]);
  ...
  function openNew(t: ReportType) { setEditing(null); setFormType(t); }
  function openEdit(r: WithId<AcademyReportDoc>) { const t = getReportType(r.type); if (t) { setEditing(r); setFormType(t); } }
  ...
  return (
    <div>
      ...
      <p className="-mt-2 mb-4 max-w-3xl text-sm text-slate-500">
        ...
        choose a report, fill it out, and save it to that class.
      </p>
